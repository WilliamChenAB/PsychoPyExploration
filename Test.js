...
function BaselineSelfReportRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of BaselineSelfReportComponents)
...
function ReadyCheckRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of ReadyCheckComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function PostTaskSelfReportRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of PostTaskSelfReportComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function NeutralizeRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of NeutralizeComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
